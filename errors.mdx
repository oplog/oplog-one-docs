...
```json
{
  ...
  "errors": [
    {
      "errorCode": "EmptyAddressFirstName",
      "errorId": 1,
      "message": "AddressFirstName field is required.",
      "field": "shippingAddress.addressFirstName"
    }
  ]
}
```
...
<Info>
**Error Handling Best Practice**: Always check the `errorCode` and `errorId` fields to programmatically handle specific error scenarios in your application.
</Info>